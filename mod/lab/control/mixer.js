...
function scheduleNextTrack() {
    ...
    const trackName = lib.math.rnde(playlist)
    log('[mixer] selecting next track from the playlist: ' + trackName)
...
function play( trackName, loop, now ) {
    ...
    const schedule = function() {
        if (!env._touched || !env.opt.music) return
...
function resume() {
    if (!env.touched || !env.opt.sound || !env.opt.music) return
    if (!track) {
        scheduleNextTrack()
        return
    }
